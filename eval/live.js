...
const scenarios = [
  ...
  { id: "unsupported-auto-data", turns: [margaretAuto, "What is the name and phone number of the adjuster assigned to this claim?"], check: ({ session, results }) => { const text = results.at(-1).text; const admitsMissingData = /don't have|do not have|not provided|not available|not listed|isn't provided|isn't available|isn't listed|doesn't include|no .*information/i.test(text); const fabricatedPhone = /(?:\+?1[ .-]?)?\(?\d{3}\)?[ .-]\d{3}[ .-]\d{4}/.test(text); return session.resolvedCaseId === "CL-2102" && admitsMissingData && !fabricatedPhone; } },
