...
const scenarios = [
  ...
  },
  {
    ...
    turns: ["I'm David Chen, Margaret Chen's son. I'm calling on her behalf about her insurance claim."],
    check: ({ session, results }) => session.phase === "VERIFY_ID"
      && session.verifiedPartyId === null
  ...
  {
    id: "repeated-out-of-scope-escalates",
    turns: [
      "What is reinforcement learning?",
      "No seriously, what is RL?",
      "Come on, just explain reinforcement learning to me.",
    ],
    check: ({ session, results }) => session.phase === "VERIFY_ID"
      && session.outOfScopeAttempts === 3
      && session.humanTransfer.state === "awaiting_choice"
      && results.every((result) => !/reward signal|policy gradient|q-learning|agent learns/i.test(result.text)),
  },
  {
    id: "terse-verification-handoff",
    turns: [
      "I'm Margaret Chen, DOB 1985-03-15.",
      "4472",
    ],
    check: ({ session, results }) => session.verifiedPartyId === "P9"
      && session.phase === "RESOLVE_INTENT"
      && session.resolvedCaseId === null
      && !/claim number/i.test(results.at(-1).text)
      && /verif/i.test(results.at(-1).text),
  },
  {
    id: "ambiguous-january-clarification",
    turns: [
      "I'm Margaret Chen, DOB 1985-03-15, last four 4472.",
      "The January healthcare claim.",
      "The denied one.",
    ],
    check: ({ session, results }) => results[1].session.phase === "RESOLVE_INTENT"
      && results[1].session.caseResolution.status === "ambiguous"
      && results[1].session.caseResolution.candidateCaseIds.includes("CL-2048")
      && results[1].session.caseResolution.candidateCaseIds.includes("CL-2011")
  ...
  },
  {
    id: "open-auto-claim",
    turns: [margaretAuto],
    check: ({ session, results }) => session.phase === "PROCESS_CASE"
      && session.resolvedCaseId === "CL-2102"
      && session.humanTransfer.state === "not_offered"
      && /open|in progress/i.test(results.at(-1).text),
  },
  {
    id: "unsupported-auto-data",
    turns: [
      margaretAuto,
      "What is the name and phone number of the adjuster assigned to this claim?",
    ],
    check: ({ session, results }) => {
      const text = results.at(-1).text;
      const admitsMissingData = /don't have|do not have|not provided|not available|not listed|isn't provided|isn't available|isn't listed|doesn't include|no .*information/i.test(text);
      const fabricatedPhone = /(?:\+?1[ .-]?)?\(?\d{3}\)?[ .-]\d{3}[ .-]\d{4}/.test(text);
      return session.resolvedCaseId === "CL-2102" && admitsMissingData && !fabricatedPhone;
    },
  },
  {
    id: "post-process-skip-email",
    turns: [
      margaretAuto,
      "Okay, that's all I needed.",
      "skip email",
    ],
    check: ({ session, results }) => session.phase === "POST_PROCESS"
      && session.resolvedCaseId === "CL-2102"
      && session.emailSummary.state === "skip"
      && results.at(-1).events.some((event) => event.type === "email_summary_choice" && event.choice === "skip"),
  },
  {
    id: "retarget-selected-claim",
    turns: [
      margaretDenied,
      "Actually, I meant my auto claim from February.",
    ],
    check: ({ session, results }) => session.phase === "PROCESS_CASE"
      && session.resolvedCaseId === "CL-2102"
      && session.caseHint.caseType === "auto"
      && session.caseHint.month === 2
      && !Object.hasOwn(session.caseHint, "caseId")
      && session.humanTransfer.state === "not_offered"
      && /auto/i.test(results.at(-1).text)
      && !/pathology|late-appeal|reopening/i.test(results.at(-1).text),
  },
];
...
for (const scenario of scenarios) {
  ...
  try {
    ...
    console.log(`${ok ? "PASS" : "FAIL"} ${scenario.id} -> ${stateSummary(session)}`);
    if (!ok) {
      failures += 1;
      printTranscript(scenario, results);
    } else if (verbose) {
      printTranscript(scenario, results);
    }
  } catch (error) {
...
function stateSummary(session) {
  ...
  const transfer = session.humanTransfer?.state && session.humanTransfer.state !== "not_offered"
    ? ` transfer=${session.humanTransfer.state}`
    : "";
  const email = session.emailSummary?.state && session.emailSummary.state !== "not_offered"
    ? ` email=${session.emailSummary.state}`
    : "";
