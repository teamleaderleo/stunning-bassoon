...
const scenarios = [
  ...
  { id: "mixed-scope", turns: ["My name is Margaret Chen. Also, what is reinforcement learning?"], check: ({ session, results }) => session.identity.name === "Margaret Chen" && session.phase === "VERIFY_ID" && session.outOfScopeAttempts === 0 && results.at(-1).plan.scope.mode !== "answer" },
  ...
  { id: "retarget-selected-claim", turns: [margaretDenied, "Actually, I meant my auto claim from February."], check: ({ session, results }) => session.phase === "RESOLVE_INTENT" && session.resolvedCaseId === "CL-2102" && session.intent === null && session.caseHint.caseType === "auto" && session.caseHint.month === 2 && !Object.hasOwn(session.caseHint, "caseId") && /auto|help|what/i.test(results.at(-1).text) && !/pathology|late-appeal|reopening/i.test(results.at(-1).text) },
  { id: "natural-human-transfer", turns: [margaretAuto, "I'd like to speak with a representative, please."], check: ({ session, results }) => session.humanTransfer.state === "requested" && results.at(-1).events.some((event) => event.type === "human_transfer_requested") && /record|request|representative|human/i.test(results.at(-1).text) },
  { id: "claim-switch-vs-secondary-mention", turns: [margaretDenied, "Does my February auto claim affect this healthcare claim?", "Actually, I meant my auto claim from February."], check: ({ session, results }) => results[1].session.resolvedCaseId === "CL-2048" && !results[1].events.some((event) => event.type === "case_retargeted") && session.phase === "RESOLVE_INTENT" && session.resolvedCaseId === "CL-2102" && session.intent === null && results.at(-1).events.some((event) => event.type === "case_retargeted") },
  { id: "principal-replacement", turns: [margaretAuto, "I gave you the wrong identity. I'm Ava Lopez.", "My DOB is 1990-08-21 and my SSN last four is 9180."], check: ({ session, results }) => results[1].session.verifiedPartyId === null && results[1].session.resolvedCaseId === null && JSON.stringify(results[1].session.identity) === JSON.stringify({ name: "Ava Lopez" }) && session.verifiedPartyId === "P7" && session.resolvedCaseId === null },
  { id: "late-representative-disclosure", turns: [margaretAuto, "Actually, I'm Margaret's son and I'm calling on her behalf."], check: ({ session, results }) => session.verifiedPartyId === null && session.resolvedCaseId === null && session.callerRole === "representative" && session.humanTransfer.state === "awaiting_choice" && results.at(-1).events.some((event) => event.type === "authorization_revoked" && event.reason === "representative_disclosure") && !/3200|3500|CL-2102/i.test(results.at(-1).text) },
];
